...
`playwright install chromium` is not optional if you want `scrape_url` to actually work — `pip install crawl4ai` only installs the Playwright Python bindings, never the browser binary itself (Playwright always requires this as a separate explicit download, regardless of project). Skipping it doesn't error loudly: `scrape_url` fails with `BrowserType.launch: Executable doesn't exist`, logs a warning, and the agent silently falls back to `fetch_url` (no JS rendering) per the priority order — so scrapes of JS-heavy sites will look "successful" but return much thinner content (e.g. just page `<title>`/`<meta description>`) with no obvious error surfaced to the end user.
...
**If that provider's model IDs contain a literal `/`** (e.g. Groq's `groq/compound`, OpenRouter's `openrouter/openai/gpt-4o`), no extra work is needed — `scraper/agent.py`'s `_MODEL_PROVIDER` (a `MultiProvider(unknown_prefix_mode="model_id", openai_use_responses=False)`) already passes such strings through as-is instead of misreading the `/` as an SDK routing prefix (see `docs/ARCHITECTURE.md`'s `scraper/agent.py` section). Just set `SCRAPER_MODEL` to the provider's real model ID, slash and all.

**Chat Completions over Responses API, by design.** `_MODEL_PROVIDER` also sets `openai_use_responses=False`, overriding the SDK's default (`use_responses=True` — OpenAI's newer Responses API). Multiple tool-calling failures during development — malformed `<function=name>{...}</function>` text artifacts, Python-style `"True"`/`"False"` strings instead of JSON booleans, models forwarding whole `ScrapeSpec` fields into tool calls — all traced back to going through Groq's Responses-API compatibility layer rather than their far more mature Chat Completions endpoint, and this was true across more than one model (not a single flaky model). If you add a new provider and hit similar tool-calling weirdness (malformed generations, loose typing, schema confusion), check whether it's happening over Responses vs. Chat Completions before assuming it's model-specific — the fix may already be in place, or the provider may need the same treatment. The defensive prompt rules and `bool | str` coercion added for the individual symptoms (see `scraper/tools/crawl4ai_tool.py`'s `_coerce_bool()`, and the RULES section of `_BASE_INSTRUCTIONS`) are still worth keeping as defense in depth even with Chat Completions forced.
...
- **Missing Playwright browser binary degrades silently, not loudly.** If `playwright install chromium` was never run, `scrape_url` fails and the agent falls back to `fetch_url` per the documented priority order — which is *correct* fallback behavior, but means a fresh environment missing this one setup step produces plausible-looking, successful-status output that's actually far thinner than intended (no JS rendering). If scraped `data` looks suspiciously sparse (e.g. only a page title/description), check `tools_used` in the output first — if it says `fetch_url` instead of `scrape_url`, this is almost always why.
...
- **Output-path recovery reads the actual `save_result` tool-call output** from `result.new_items` (correlated by `call_id`), not the model's final chat message — a text-scan of `final_output` is kept only as a fallback. If you change `save_result`'s return string format in `output_tool.py`, update the `"Saved:"` prefix check in `scraper/agent.py::run()` to match, or successful saves will stop being recognized. (This used to be pure string-matching against the model's final text, which some models don't reliably repeat — that was a real bug, now fixed; keep the tool-call-based path as primary if you touch this again.)
