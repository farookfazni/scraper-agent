...
scraper-agent inverts this: there is **one LLM agent** with a fixed toolbox (10 Python functions + whatever MCP servers are configured), and a **system prompt** that tells the agent, in natural language, what order to try tools in for a given `site_type`, and what to do when a tool fails. The agent — not a Python dispatcher — decides which tool to call next, based on the prompt and the tool's return value.
...
| paginated listing (detected after 1st scrape) | — | MCP `firecrawl_crawl` → `crawl_paginated` (crawl4ai deep-crawl) → manual `scrape_urls` (last resort) |
| deep research | — | MCP `firecrawl_deep_research` |

**Pagination is detected automatically, not just on request** — but is now handled by a deterministic tool, not prompt-guessed URL construction. After the first `scrape_url` call, the agent checks the content for pagination signals (page numbers, Next/Prev links, "Page X of Y") and — if the user's goal implies wanting the complete set — calls `crawl_paginated(start_url, max_pages, url_pattern)`, which uses crawl4ai's built-in `BFSDeepCrawlStrategy` to actually follow the real pagination links (see §4's `crawl4ai_tool.py` entry). This replaced an earlier, unreliable approach where the agent was told to infer the page-URL pattern itself and call `scrape_urls` across a manually-built list — in testing, this reliably returned only page 1's items even when `scrape_urls` was attempted, since correctly inferring the URL pattern and constructing every page URL is exactly the kind of mechanical task an LLM shouldn't be trusted to get right every time. Manual `scrape_urls` construction is kept only as a last-resort fallback.
...
**`ScraperAgent.__init__`** builds an `Agent` (from the `agents` SDK) with `name="ScraperAgent"`, `model` (default `gpt-4o-mini`, overridable via `SCRAPER_MODEL`), `instructions=_compose_instructions(mcp_servers)` (§7), and the 10 always-on tools plus whatever `mcp_servers` were passed in. All 10 Python tools are registered unconditionally — even ones whose API key is missing — because each tool checks its own prerequisites at call time and returns `TOOL_UNAVAILABLE: ...` rather than needing to be conditionally excluded at construction time.

**`ScraperAgent.run(spec)`** does three things:
1. Serializes `spec` into the prompt and calls `Runner.run(self._agent, input=prompt, max_turns=30, run_config=RunConfig(model_provider=_MODEL_PROVIDER))` — this is the actual agent loop (LLM call → tool execution → feed result back → repeat). `_MODEL_PROVIDER` is a `MultiProvider(unknown_prefix_mode="model_id", openai_use_responses=False)` with two fixes bundled in:
   - `unknown_prefix_mode="model_id"` — needed because some providers use `/` as part of a literal model ID (Groq's `groq/compound`, OpenRouter's `openrouter/openai/gpt-4o`), which the SDK's default `MultiProvider` otherwise misreads as an unrecognized routing prefix and rejects with `UserError: Unknown prefix: ...`.
   - `openai_use_responses=False` — forces the classic Chat Completions API (`/chat/completions`) instead of the SDK's default, OpenAI's newer Responses API (`/responses`, `_use_responses_by_default = True` at the SDK level). Chat Completions' tool-calling format is the far more universal, mature standard across OpenAI-compatible providers; several distinct tool-calling failures hit during development (malformed `<function=...>` text artifacts, loose boolean serialization, spec-field forwarding) all trace back to going through Groq's Responses-API compatibility layer rather than their more battle-tested Chat Completions endpoint. This is the actual root fix — the earlier prompt-level guardrails for individual symptoms (documented below) are still worth keeping as defense in depth, but this is what addresses the underlying cause.
2. Recovers the saved output path from `result.new_items` — it finds the `ToolCallItem` whose `tool_name == "save_result"`, then the matching `ToolCallOutputItem` with the same `call_id`, and reads that item's `.output` directly (the literal string `save_result` returned, e.g. `"Saved: <path>"`). This is the actual tool-call trace, not the model's free-text final answer — some models' last chat message doesn't reliably repeat "Saved: ..." verbatim even when the save succeeded (observed with Groq's `compound`, whose final message rambled about "the context" instead), which used to cause `ScraperAgent.run()` to report `status="failed"` on runs that had actually saved correctly. A text-scan of `result.final_output` is kept only as a fallback in case the tool-call trace can't be correlated for some reason.
3. If neither recovery path finds a valid, existing file, it returns `ScrapeOutput(status="failed", errors=[...])` with the agent's raw output (truncated to 200 chars) as the error message — this is now a genuine failure signal, not a false negative from text-parsing.

### `scraper/tools/*.py` — the 10 tools
...
- **`crawl_paginated`** — deterministic multi-page crawl for numbered pagination, via crawl4ai's `BFSDeepCrawlStrategy` (`max_depth=1` fixed, `max_pages` + an optional `URLPatternFilter`/`DomainFilter` glob to scope which links get followed). Returns the same `{url, status, content}` array shape as `scrape_urls`. Confirmed gotcha: with `deep_crawl_strategy` set and `stream=False`, `crawler.arun()` returns a plain `list[CrawlResult]` rather than the single-result container `scrape_url`/`scrape_urls` assume — the tool's implementation iterates that list directly. This exists because prompt-only pagination handling (telling the agent to infer a URL pattern and call `scrape_urls` itself) proved unreliable in testing — it reused `scrape_urls` but still only returned page 1's data. Not cached (`@cached_tool`), matching `scrape_urls`' precedent — a multi-page crawl shouldn't silently return a stale result.
...
- **Output-path recovery** now reads `save_result`'s actual return value from the tool-call trace (`result.new_items`, correlated by `call_id`) rather than parsing the model's final chat message — see `scraper/agent.py`'s `run()`. A text-scan of `final_output` remains only as a fallback. This closed a real bug where some models' final message didn't repeat the save path, causing false `status="failed"` reports on runs that had actually succeeded.
