...
One agent, a composable tool layer, a fixed output schema.

```text
main.py  (CLI / REPL)
  └── ScraperAgent  (scraper/agent.py)
        ...
        ├── firecrawl_search    — requires FIRECRAWL_API_KEY
        ├── fetch_url           — plain HTTP fallback
...
| `scraper/agent.py` | `ScraperAgent` class — wraps the SDK `Agent`, holds tool list, parses output |
...
| `scraper/tools/search_tool.py` | `tavily_search`, `duckduckgo_search`, `firecrawl_search` |
...
| `FIRECRAWL_API_KEY` | — | Enables `firecrawl_search` |
...
| --- | --- | --- |
| `pdf` | skip | pdf_extract → scrape_url → fetch_url |
| `table` | skip | extract_structured → scrape_url |
| `financial` | tavily → ddg | scrape_url → fetch_url |
| `news` | tavily → ddg | scrape_url → fetch_url |
...
- **New tool** — add a `@function_tool` function in `scraper/tools/`, export from `scraper/tools/__init__.py`, add to `all_tools` in `scraper/agent.py`.
- **New provider** — add `base_url` mapping in `main.py`'s `_setup_client()`.
- **Custom output fields** — extend `ScrapeSpec.extract_fields`; the JSON envelope is fixed, but `data` is open.
